...
Purpose:
    ...
    contained HTML file (inventory.html) you can double-click to view
...
Run this manually with:  python build_inventory_page.py
Then double-click inventory.html (or open it in a browser) to view it.
"""
...
# sync with build_home_page.py / compare_to_other_rate.html
# if any of them is ever renamed again.
HOME_PAGE = "home.html"
INVENTORY_PAGE = "inventory.html"
COMPARE_PAGE = "compare_to_other_rate.html"
OUTPUT_FILE = os.path.join(SCRIPT_DIR, INVENTORY_PAGE)
...
<title>Our Gold full details</title>
<style>
  :root {
    color-scheme: light;
    --page-plane:     #f9f9f7;
...
<nav class="top-nav">
  ...
  <div class="nav-links">
    <a href="__HOME_PAGE__" class="nav-link">Home</a>
    <a href="__INVENTORY_PAGE__" class="nav-link active">Our Gold full details</a>
...
<div class="dashboard">
  ...
  <footer class="dash-footer">Generated by build_inventory_page.py on __GENERATED_AT__</footer>
</div>
...
<script>
  // Auto-refresh: unlike compare_to_other_rate.html, this page's rows are
  // baked into the HTML at build time (see build_html() below), not
...
def build_html(rows: list, totals: dict, todays_rate: float) -> str:
    """
    ...
    subtitle = (
        ...
        f"Using 22K rate {format_inr(todays_rate)}/gram"
    )
...
def main():
    try:
        ...
        # -----------------------------------------------------------------
        html = build_html(rows, totals, todays_rate)
